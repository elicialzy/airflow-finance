...
1. insert_many for collection, need to insert into both cloud and local (DONE)
2. drop all collections in all databases, need to drop both cloud and local (DONE)
3. find collections for databases, and be able to pass filters (DONE)
...
OUT_FILE_PATH = "/home/airflow/3107-pipeline/tmp"
class DBHelper:
    ...
    def get_documents_for_collection(self, database_name, collection_name, run_cloud = True, filter={}):
        if self.validate:
            self.__validate__(database_name, collection_name)

        try:
            if run_cloud:
                return list(self.cloud_client.get_database(name=database_name).get_collection(name=collection_name).find(filter))
            else:
                return list(self.local_client.get_database(name=database_name).get_collection(name=collection_name).find(filter))
        except Exception as e:
            if run_cloud: # Cloud failed, tried local
    ...
    def insert_many_for_collection(self, database_name, collection_name, records):
        ...
        for record in records:
            record['created-at'] = datetime.now(tz=pytz.UTC)
    ...
    def clean_database(self):
        ...
        for database in database_names:
            collection_names = self.get_collection_names_for_database(database)
            for collection in collection_names:
                deleted = self.delete_documents_for_collection(database, collection)
                if not deleted:
                    all_deleted = False
        
        if not all_deleted:
            print(f"{DEBUG_ENV}-{DEBUG_LOCAL}: Clean Database Failed, Some Collections Are Not Cleared\n")
        else:
            print(f"{DEBUG_ENV}: Clean Database Success\n")
    ...
    def export_collection_to_json(self, database_name, collection_name):
        ...
        for document in documents:
            del document["_id"]
            if document["created-at"]:
                document["created-at"] = document["created-at"].isoformat()
        ...
        try: 
            ...
            with open(f"{OUT_FILE_PATH}/{database_name}/{collection_name}.json", "w+") as f:
